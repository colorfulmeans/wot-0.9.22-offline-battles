"""Static navigation removal contracts; not a native game-play or FPS test."""
...
SCRIPTS = os.environ.get('IMMUTABLE_CLIENT_SOURCE', os.path.join(ROOT, 'src', 'res', 'scripts', 'client'))
...
class ImmutableNavigationTests(unittest.TestCase):
    ...
    def test_contact_reporting_no_longer_enrols_a_region(self):
        navigator,calls=self.make()
        navigator.bot_states[101]={'blocked_step_replans':0,'replan_generation':0}
        navigator.report_hard_contact(101,(20.,0.,28.),(24.,0.,28.),math.pi/2.,1.)
        self.assertIn('hard_contact_episode',navigator.bot_states[101])
        self.assertFalse(calls)

    def test_read_only_displaced_hull_connector_keeps_wall_checks(self):
        graph=graph_sample([(0,z) for z in range(15)])
        calls=[]
        def hit(*args): calls.append(args);return True
        grid=TerrainGrid(lambda *args:0.,hit,baked_graph=copy.deepcopy(graph))
        before=copy.deepcopy((grid._baked_heights,grid._baked_links))
        self.assertFalse(grid.segment_clear((0.,0.,28.),(4.,0.,28.)))
        self.assertTrue(calls)
        grid.obstacle_probe=lambda *args:False
        self.assertTrue(grid.segment_clear((0.,0.,28.),(4.,0.,28.)))
        self.assertEqual(before,(grid._baked_heights,grid._baked_links))
    ...
    def test_baked_climb_approach_is_retained(self):
        self.assertFalse(TerrainGrid.shortcut_preserves_climb_approach(
            [(0.,0.,0.),(0.,0.,10.),(10.,3.,10.)],0,2))
    ...
    def test_runtime_reset_has_no_deleted_callback(self):
        navigator,calls=self.make()
        navigator.invalidate_native_planning()
        navigator.grid.clear_negative_cache()
        navigator.fallback_diagnostics(now=1.)
        navigator.bot_search_diagnostics(101,(8.,0.,8.),1.)
        self.assertFalse(calls)
...
def run_suite():
    ...
    output=os.environ.get('IMMUTABLE_TEST_REPORT',os.path.join(ROOT,'immutable-navigation-tests.json'))
