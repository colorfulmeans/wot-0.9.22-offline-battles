...
def bytecode():
    ...
    temp=tempfile.mkdtemp(prefix='immutable-pyc-')
    try:
        with zipfile.ZipFile(package) as z:
            ...
            z.extractall(temp)
        os.environ['IMMUTABLE_CLIENT_SOURCE']=os.path.join(temp,'res','scripts','client')
        os.environ['IMMUTABLE_TEST_REPORT']=os.path.join(ROOT,'dist','immutable-bytecode-tests.json')
        sys.path.insert(0,os.path.join(ROOT,'tools'))
        import test_immutable_navigation as tests
        assert tests.run_suite(),'compiled navigation removal regressions failed'
        ...
        save(os.path.join(ROOT,'dist','immutable-bytecode-verification.json'),result)
        print('PASS: %d exact Python 2.7 modules and compiled removal tests'%len(verified))
    finally:shutil.rmtree(temp)

def distribution(app):
    app=os.path.abspath(app);payload=os.path.join(app,'_internal')
    proof=load(os.path.join(ROOT,PROOF));compiled=load(os.path.join(ROOT,'dist','immutable-bytecode-verification.json'))
    ...
    import core
    fake=tempfile.mkdtemp(prefix='immutable-install-')
    try:
    ...
    result={'source_commit':os.environ['IMMUTABLE_SOURCE_SHA'],'github_run_id':os.environ['GITHUB_RUN_ID'],
        'build_identity':identity,'verified_graphs':41,'base_runtime':'dcbc5b686bb8f522175fc80d38cade4f756ccca6',
    ...
    save(os.path.join(app,'NO_NATIVE_REVIEW_BUILD_EVIDENCE.json'),result)
