...
class VehicleOverlayTest(unittest.TestCase):
    ...
    def test_roster_reports_the_stock_class_tag_and_tier_for_each_vehicle(
        ...
        self.assertEqual(
            ("lightTank", 8), (choices["R12_Test"]["vehicleClass"],
    ...
    def _prepare_isolated_worker_fixture(self):
        self._write('paths.xml', (
            '<root><Paths><Path>./res_mods/0.9.22.0.1</Path>'
            '<Path>./mods/0.9.22.0.1</Path>'
            '<Path>./res/packages/scripts.pkg</Path>'
            '<Path>./res</Path></Paths></root>').encode('utf-8'))
        package = self._write(
            'mods/0.9.22.0.1/org.peng.offline_lan_0922_1.0.wotmod', b'')
        with zipfile.ZipFile(package, 'w') as archive:
            archive.writestr(
                'res/scripts/client/gui/mods/mod_offline_lan_0922.py', b'pass')
        self._write(
            'res_mods/0.9.22.0.1/engine_config.offline-worker.xml', b'<root/>')
        core.prepare_worker_resource_root(self.game)

    def test_worker_start_uses_current_owned_profile_without_other_mods(self):
        import wot_launcher

        self._prepare_isolated_worker_fixture()
        other_mod = 'scripts/client/gui/mods/mod_other.py'
        self._write('res_mods/0.9.22.0.1/' + other_mod, b'other mod')
        for name, member, field, value in (
                ('Fast', self.VEHICLE, 'speedLimits/forward', '40'),
                ('Strong', self.ENGINES, 'shared/GAZ-M1/power', '120')):
            vehicle_overlays.create_vehicle_profile(self.game, name)
            vehicle_overlays.apply_profile_edit(
                self.game, name, member, field, value, is_running=lambda: False)

        root = core.worker_resource_root(self.game)
        for profile, active_member in (
                ('Fast', self.VEHICLE), ('Strong', self.ENGINES), (None, None)):
            # The launcher installs its package before activating the profile.
            # Worker startup must rebuild after that activation, including when
            # switching back to stock or to a disjoint set of profile members.
            vehicle_overlays.prepare_vehicle_profile(
                self.game, profile, is_running=lambda: False)
            self.assertIsNone(core.worker_resource_path_list(self.game))
            window = mock.Mock(_stop_requested=False)

            def attempt(*unused_args, **unused_kwargs):
                self.assertIsNotNone(core.worker_resource_path_list(self.game))
                for member in (self.VEHICLE, self.ENGINES):
                    destination = os.path.join(root, *member.split('/'))
                    if member == active_member:
                        with open(destination, 'rb') as stream:
                            worker_bytes = stream.read()
                        with open(self._overlay(member), 'rb') as stream:
                            self.assertEqual(stream.read(), worker_bytes)
                    else:
                        self.assertFalse(os.path.exists(destination))
                self.assertFalse(os.path.exists(os.path.join(root, other_mod)))
                return True

            window._start_worker_attempt.side_effect = attempt
            self.assertTrue(wot_launcher.LauncherWindow._start_worker(
                window, self.game, '127.0.0.1', 28782))
            window._start_worker_attempt.assert_called_once()

    def test_modified_owned_overlay_cannot_reuse_the_isolated_worker_root(self):
        self._prepare_isolated_worker_fixture()
        vehicle_overlays.create_vehicle_profile(self.game, 'Fast')
        vehicle_overlays.apply_profile_edit(
            self.game, 'Fast', self.VEHICLE, 'speedLimits/forward', '40',
            is_running=lambda: False)
        vehicle_overlays.prepare_vehicle_profile(
            self.game, 'Fast', is_running=lambda: False)
        core.prepare_worker_resource_root(self.game)
        self.assertIsNotNone(core.worker_resource_path_list(self.game))
        with open(self._overlay(self.VEHICLE), 'wb') as stream:
            stream.write(b'changed outside the owned profile')
        self.assertIsNone(core.worker_resource_path_list(self.game))
        actions = core.prepare_worker_resource_root(self.game)
        self.assertTrue(any('changed' in action for action in actions), actions)
        self.assertIsNone(core.worker_resource_path_list(self.game))
