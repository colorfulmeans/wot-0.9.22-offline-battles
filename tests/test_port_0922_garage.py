...
class GaragePersistenceTests(unittest.TestCase):
    ...
    def test_native_vehicle_and_crew_bonuses_keep_distinct_owners(self):
        ...
        vehicles, tankmen = _modules()
        original_get_type = vehicles.getVehicleType
        def get_type(compact_descr):
            result = original_get_type(compact_descr)
            result.premiumVehicleXPFactor = 0.5
            result.crewXpFactor = 1.5
            return result
        vehicles.getVehicleType = get_type
        store = self._store()
        def settle():
            return store.apply_battle_crew_xp(
                snapshot, 'server:premium:1', PREMIUM_VEHICLE_CD, 200, 1,
        ...
        self.assertEqual(600, _TankmanDescriptor(
            snapshot['vehicles'][0]['tankmen'][101]).totalXP())
        before = copy.deepcopy(snapshot)
        duplicate = settle()
        self.assertFalse(duplicate['applied'])
        self.assertEqual(result['awarded'], duplicate['awarded'])
