...
class LanProtocolTests(unittest.TestCase):
    ...
    def test_v5_explicit_control_messages(self):
        ...
        self.assertEqual(880, self.sent[4]['damage_to_target'])
        self.assertEqual('rules_state', self.sent[5]['type'])
        self.assertEqual('destructible', self.sent[6]['type'])
        self.assertEqual('tree', self.sent[6]['destructible_kind'])
        self.assertEqual(17, self.sent[6]['chunk_id'])
    ...
    def test_retired_direct_damage_report_apis_are_removed(self):
        worker = self._worker_client()
        for client in (self.client, worker):
            for method_name in (
                    'send_hit', 'send_bot_hit', 'send_bot_human_hit',
                    'send_bot_bot_hit', 'send_spotted_report'):
    ...
    def test_assist_event_and_result_statistics_are_json_safe(self):
        ...
        self.assertEqual({
            ...
            'explosion_hits_received', 'explosion_hits',
        ...
        # integer key would stop it round-tripping.
        self.assertTrue(
            all(isinstance(key, str) for key in rows[1]['shells_fired']))
