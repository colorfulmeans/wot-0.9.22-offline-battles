...
ROUND_SCOPED_MESSAGE_TYPES = frozenset((
    "start_battle", "input", "bot_manifest", "bot_state",
    "bot_observation", "bot_ram_report", "fire_intent",
    "fire_intent_result",
...
def _compatible_hello_protocol(value):
    """Accept only the protocol shipped by the single #1513 client line."""
    return bool(
...
def _critical_payload(value):
    """Validate one authority-resolved critical-state transition.
...
class BattleState:
    ...
    def _message_round_matches(self, message):
        """Require an exact current-round fence on every battle message."""
    ...
    def _new_bot_roster(self, occupied_slots=None):
        ...
        roster = []
        used = set()
        available_callsigns = list(BOT_CALLSIGNS_0922)
    ...
    def add_player(self, conn, address, hello):
        with self.lock:
            ...
            if account_key is None:
                # Protocol-v5 #1513 clients released before durable receipts
                # did not send this field. Current clients always send it.
                fallback = "".join(
                    character for character in str(
                        hello.get("name", "player"))
                    if character.isalnum())[:48]
                account_key = "legacy_%s" % (fallback or "player")
            ...
            if any(participant.account_key == account_key
                ...
                # The account key owns durable receipts. Two live players may
                # not share it and overwrite one participant's frozen result.
    ...
    def request_start(self, player_id, requested_map=None,
        ...
        with self.lock:
            ...
            if (any(HUMAN_RAM_TIMELINE_CAPABILITY not in
            ...
            if (any(RAM_CONTACT_LEDGER_CAPABILITY not in
                         participant.capabilities
                         for participant in self.players.values()
                         if participant.connected) or
                     self.simulation_worker is None or
            ...
            if (any(EFFECTIVE_PARAMS_CAPABILITY not in
                         participant.capabilities
                         for participant in self.players.values()
                         if participant.connected) or
                     self.simulation_worker is None or
            ...
            if (any(RICOCHET_CONTINUATION_CAPABILITY not in
                         participant.capabilities
                         for participant in self.players.values()
                         if participant.connected) or
                     self.simulation_worker is None or
    ...
    def _sanitize_bot_ammo(raw, identity, previous):
        ...
        # Production manifests carry descriptor shell summaries. Focused
        # engine-free harnesses may omit them; the atomic inventory is still
        # self-sizing and bounded, so preserve it without inventing a shell
    ...
    def update_bot_states(self, player_id, message):
        ...
        with self.lock:
            ...
            # serialization duration out of the pose velocity.
    ...
    def report_bot_ram(self, player_id, message):
        ...
        with self.lock:
            ...
            requires_contact = target_kind == "human"
        ...
        with self.lock:
            ...
            if player.alive:
                ...
                if (RAM_CONTACT_LEDGER_CAPABILITY in player.capabilities and
                    ...
                    # Current senders may repeat one latest receipt when no
                    # batched contact ledger is available.
                ...
                if raw_contacts is not None:
                    ...
                    for seq in sorted(by_seq):
                        ...
                        if contact is None:
                            ...
                            continue
                        # The contact body is sampled before local separation,
                        # while this input's ordinary pose is sampled after the
                        # reciprocal impulse/correction. Replacing the former
                        # with the latter therefore removes the overlap that the
                        # authority must independently re-check and turns every
                        # player-bot ram into a terminal zero-damage receipt.
                        # Bind the validated pre-separation body to this ordered
                        # input transaction without rewriting it; the worker
    ...
    def _record_frag(self, attacker_kind, attacker_id, victim_team,
        ...
        if attacker_kind == "player":
            ...
            frags = actor["frags"]
            # Bots adjust frags, but only the human player path publishes the
            # blue/team-killer state.
            team_killer = False
        else:
            return False
        if delta < 0:
    ...
    def _combat_accepting(self):
        """Fence combat until the shared countdown becomes live."""
    ...
    def _update_capture(self):
        ...
        if (not self._combat_accepting() or
                ...
                self.battle_result is not None):
            return False
        # Navigation graphs contain the decoded CTF objective positions. Route
        # authoring data is never a capture-circle fallback.
        bases = self.capture_bases
        if not bases:
            return False
        vehicles = {1: [], 2: []}
        for player in self.players.values():
    ...
    def tick_once(self, dt):
        ...
        with self.lock:
            ...
            # state, ordered events and expiration edges.
            ...
            for ordinal, pending in enumerate(self.pending_events):
                ...
                events.append(event)
            self.pending_events = []
            # Events and the snapshot published by one simulation tick share
            # the current clock sample frozen above. Reusing a prior snapshot's
            # time would make delayed projectile tracers start at the wrong
            # point on their authoritative trajectory.
            snapshot = {
                "type": "snapshot",
                "protocol": PROTOCOL_VERSION,
                "server_tick": self.tick,
    ...
    def handle(self):
        ...
        try:
            ...
            if (not isinstance(hello, dict) or hello.get("type") != "hello" or
                    not _compatible_hello_protocol(hello.get("protocol"))):
            ...
            if role == "probe":
                ...
                raw_capabilities = hello.get("capabilities", ())
                valid_client_build = client_build == CLIENT_BUILD_0922
                if not valid_client_build:
                    self._send_raw(conn, {
                        "type": "error",
                        "code": "unsupported_client_build",
                        "message": "launcher probe has the wrong client build",
                    })
            ...
            with server.state.lock:
                ...
                if player is not None:
                    ...
                    welcome_message.update(server.state._authority_fields())
                ...
                while b"\n" in buffer:
                    ...
                    try:
                        ...
                        if (message_type in ROUND_SCOPED_MESSAGE_TYPES and
                                not server.state._message_round_matches(message)):
                            continue
                        if message_type not in MODERN_VISIBLE_MESSAGE_TYPES:
                            _server_log_limited(
                                "visible-command:%d:%s" % (
