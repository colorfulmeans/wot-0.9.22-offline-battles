...
# here. Vehicle XP and crew XP bonuses come from their separate exact
# descriptor fields rather than this credit policy.
...
def premium_vehicle_xp_factor_100(vehicles_module, vehicle_type_compact_descr):
    """Read the exact #1513 vehicle XP bonus, separately from crew XP."""
    try:
        vehicle_type = vehicles_module.getVehicleType(
            int(vehicle_type_compact_descr))
...
def premium_xp_bonus(value, factor_100):
    """Match the embedded Python 2.7 ValueReplay coefficient rounding."""
    amount = max(0, int(value)) * max(0, int(factor_100)) / 100.0
    # Inputs are nonnegative. Python 3's round-to-even differs at half steps.
