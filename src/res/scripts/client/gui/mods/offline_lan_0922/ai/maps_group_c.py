# -*- coding: utf-8 -*-
"""Conservative macro routes retained as reviewed lane sketches.

This is deliberately data-only. Exact objectives and team starts come from the
pinned #1513 arena data during navigation baking. A waypoint's third value is
...
def _map(name, bounds, routes):
	"""Make independent forward and reverse copies of every lane sketch."""
	team_one_routes = []
	for route in routes:
		forward = dict(route)
		forward['role_weights'] = dict(route['role_weights'])
		forward['waypoints'] = tuple(route['waypoints'])
		team_one_routes.append(forward)
	reversed_routes = []
	for route in team_one_routes:
		other = dict(route)
	...
	return {
		'name': name,
		'bounds': bounds,
		'routes': {1: tuple(team_one_routes), 2: tuple(reversed_routes)},
	}
...
REDSHIRE = _map('34_redshire', (-500.0, -500.0, 500.0, 500.0),
	(
	_route('east_ridge', 'overwatch', 4, 0.57, ((368.69, -269.52, 0), (355, -145, 0), (292, -58, 1), (205, 35, 1), (90, 146, 1), (-85, 286, 0), (-209.86, 368.25, 0))),
	_route('river_town', 'line', 5, 0.72, ((368.69, -269.52, 0), (270, -245, 0), (172, -174, 1), (105, -88, 1), (23, 8, 1), (-84, 112, 1), (-165, 250, 0), (-209.86, 368.25, 0))),
	_route('west_fields', 'flank', 4, 0.49, ((368.69, -269.52, 0), (246, -340, 0), (88, -366, 0), (-92, -300, 1), (-250, -155, 1), (-325, 32, 1), (-280, 218, 0), (-209.86, 368.25, 0))),
))
...
CANADA_A = _map('47_canada_a', (-500.0, -500.0, 500.0, 500.0),
	(
	_route('west_hills', 'flank', 4, 0.61, ((-127, -306, 0), (-190, -230, 0), (-263, -238, 0), (-334, -97, 1), (-302, 64, 1), (-196, 198, 1), (23, 300, 0), (213, 328, 0))),
